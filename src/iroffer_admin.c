...
// SPDX-FileCopyrightText: 2018-2020 antiz <antiz@qt-mail.com>
//
...
static void u_dcld(const userinput* const u) {
    ...
    while (tr) {
        ...
        if (tr->tr_status == TRANSFER_STATUS_SENDING ||
            ...
            u_respond(
                u,
                "  ^- %5.1fK/s    %6jdK/%6jdK  %2i%c%02i%c/%2i%c%02i%c  "
                "%5s/%5s  %7s",
                tr->lastspeed, (intmax_t)((tr->bytessent) / 1024),
                (intmax_t)((tr->xpack->st_size) / 1024),
    ...
    while (ul) {
        ...
        if (ul->ul_status == UPLOAD_STATUS_GETTING ||
            ...
            u_respond(u,
                      "  ^- %5.1fK/s    %6jdK/%6jdK  %2i%c%02i%c/%2i%c%02i%c",
                      ul->lastspeed, (intmax_t)((ul->bytesgot) / 1024),
...
static void u_info(const userinput* const u) {
    ...
    sizestrstr = sizestr(1, xd->st_size);
    u_respond(u, " Filesize       %jd [%sB]", (intmax_t)xd->st_size,
              sizestrstr);
    mydelete(sizestrstr);
    ...
    u_respond(u, " Device/Inode   %ju/%ju", (uintmax_t)xd->st_dev,
              (uintmax_t)xd->st_ino);
...
static void u_botinfo(const userinput* const u) {
    ...
    if (irlist_size(&gdata.uploadhost)) {
        snprintf(tempstr, maxtextlength - 1, "%jdMB",
                 (intmax_t)(gdata.uploadmaxsize / 1024 / 1024));
    ...
    if (u->arg1 && !strcmp(u->arg1, "list") && mmap_count) {
        ...
        for (xd = irlist_get_head(&gdata.xdccs); xd; xd = irlist_get_next(xd)) {
            ...
            for (mm = irlist_get_head(&xd->mmaps); mm;
                ...
                u_respond(u, " %4d | 0x%8.8jX .. 0x%8.8jX | %p | %10d",
                          pack_count, (uintmax_t)mm->mmap_offset,
                          (uintmax_t)mm->mmap_offset +
                              (uintmax_t)mm->mmap_size - 1,
...
static void u_trinfo(const userinput* const u) {
    ...
    u_respond(u,
              "Start %jdK, Current %jdK, End %jdK (%2.0f%% File, %2.0f%% Xfer)",
              (intmax_t)((tr->startresume) / 1024),
              (intmax_t)((tr->bytessent) / 1024),
              (intmax_t)((tr->xpack->st_size) / 1024),
    ...
    u_respond(u, "Min %s, Current %1.1fK/s, Max %s, In Transit %jdK",
              (tr->nomin || (tr->xpack->minspeed == 0.0)) ? "no" : tempstr2,
              tr->lastspeed,
              (tr->nomax || (tr->xpack->maxspeed == 0.0)) ? "no" : tempstr3,
              (intmax_t)(tr->bytessent - tr->lastack) / 1024);
...
#ifdef HAVE_MMAP
    if (tr->mmap_info) {
        u_respond(u, "MMAP: [%p] 0x%.8jX .. 0x%.8jX .. 0x%.8jX",
                  tr->mmap_info->mmap_ptr,
                  (uintmax_t)tr->mmap_info->mmap_offset,
                  (uintmax_t)tr->bytessent,
                  (uintmax_t)tr->mmap_info->mmap_offset +
                      (uintmax_t)tr->mmap_info->mmap_size - 1);
    }
#endif
}
